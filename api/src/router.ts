...
for (const serveDir of serveDirs) {
  const getDirChildren = async (nodePath: string, fullPath: string) => {
  ...
  api.get(`/directories/${serveDir.name}`, async (req, res) => {
    reqAdminMode(req)
    res.send(await getDirChildren('', serveDir.path))
  })
  api.get(`/directories/${serveDir.name}/*path`, async (req, res) => {
    reqAdminMode(req)
    const nodePath = path.join(...(req.params as any).path as string[])
    const fullPath = path.join(serveDir.path, nodePath)
    const stats = await fs.stat(fullPath)
    if (stats.isDirectory()) {
      res.send(await getDirChildren(nodePath, fullPath))
    } else {
      const ip = reqIp(req)
